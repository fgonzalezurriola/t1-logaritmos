...
std::vector<int64_t> read_block(const std::string &filename, int64_t block_index) {
    std::vector<int64_t> buffer(INTS_PER_BLOCK);
...
void write_block(
    ...
    if (!out) {
        ...
        create.close();
        out.open(filename, std::ios::in | std::ios::out | std::ios::binary);
        if (!out) {
            std::cerr << "Error opening file " << filename << std::endl;
            exit(EXIT_FAILURE);
        }
    ...
    if (!out) {
        std::cerr << "Error writing to block in file " << filename << std::endl;
    }
}
...
void run_arity_experiment(int64_t max_arity) {
    ...
    results_out << "Arity,Time,I/O's" << endl;

    vector<ArityResult> arity_results;
    // for (int64_t arity = 2; arity <= max_arity; arity *= 2) {
    int64_t i = 0;
    int64_t start = 476;
    int64_t step = 4;
    for (int64_t arity = start; arity <= max_arity; arity = 476 + (step * i)) {
        cout << "\n=========================================================" << endl;
        cout << "Testing arity: " << arity << endl;
        cout << "Current time: " << chrono::system_clock::now().time_since_epoch().count() << endl;
        cout << "=========================================================" << endl;
        ...
        cout << "  IO Operations: " << io_operations << endl;

        results_out << arity << "," << fixed << setprecision(6) << duration << "," << io_operations
                    << endl;

        arity_results.push_back({arity, duration});
        i++;
    }

    results_out.close();

    int64_t best_arity = find_best_arity(results_file);

    cout << "Experiment completed. Results saved to " << results_file << endl;
    cout << "Best arity found: " << best_arity << endl;
}

int64_t find_best_arity(const string &results_file) {
    ifstream results_in(results_file);
    if (!results_in) {
    ...
    string header;
    getline(results_in, header);

    int64_t best_arity = 2;
    ...
    while (getline(results_in, line)) {
        ...
        if (comma1 != string::npos && comma2 != string::npos) {
            arity = stoll(line.substr(0, comma1));
            time = stod(line.substr(comma1 + 1, comma2 - comma1 - 1));
