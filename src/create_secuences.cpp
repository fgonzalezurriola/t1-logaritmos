...
void create_and_write_M(int64_t m_mult) {
    for (int j = 0; j < NUMBER_OF_SECUENCES; j++) {
        ...
        for (int i = 0; i < m_mult * BLOCKS_PER_M; i++) {
            ...
            write_vector_to_file(filename, vector);
        }
    }
}
...
vector<int64_t> random_vector_int64(int64_t n) {
    ...
    static uniform_int_distribution<int64_t>
        dist(numeric_limits<int64_t>::min(), std::numeric_limits<int64_t>::max());
    for (int i = 0; i < n; i++) {
        vec[i] = dist(rng);
    }
    return vec;
...
// TODO: la mejor forma sería crear una función que pueda "abstraer" esto
// ? Idea de flujo: crear bins para 4, ejecutar quicksort, limpiar, repertir para mergesort
// ? Por último repetir para cada múltiplo de 4 hasta 60
// ? Guardar los resultados en una outputs/output_txt_quicksort_m4 mergesort_m4... para cada x4
